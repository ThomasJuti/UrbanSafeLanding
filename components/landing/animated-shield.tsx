...
export function AnimatedShield() {
  ...
  useEffect(() => {
    ...
    // Shield silhouette vertices
    const vertices = [
      { x: 0, y: 1.1, z: 0 },       // Top point
      { x: -0.8, y: 0.5, z: 0.15 }, // Upper left
      { x: 0.8, y: 0.5, z: 0.15 },  // Upper right
      { x: -0.6, y: -0.9, z: 0.15 }, // Lower left
      { x: 0.6, y: -0.9, z: 0.15 },  // Lower right
      { x: 0, y: -1.3, z: 0 },      // Bottom tip
    ];

    // Edges connecting vertices (outline + fan diagonals)
    const edges = [
      [0, 1], [1, 3], [3, 5], [5, 4], [4, 2], [2, 0],
      [0, 3], [0, 5], [0, 4],
    ];

    // Faces for filling with points (fan triangulation from top vertex)
    const faces = [
      [0, 1, 3],
      [0, 3, 5],
      [0, 5, 4],
    ...
    const render = () => {
      ...
      points.forEach((point) => {
        ...
        ctx.fillStyle = `rgb(${glyphRgb} / ${Math.min(alpha, 0.9)})`;
